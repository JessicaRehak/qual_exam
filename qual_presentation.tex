...
\begin{frame}
  ...
  \begin{align*}
  &\left[ \oh\cdot  \nabla + \Sigma_t(\rvec,E) \right] \psi^{i+1}(\rvec,E,\oh)\\
  & \quad \quad \quad= \int_0^{\infty}dE'\int_{4\pi}d\oh'\Sigma_s(\rvec, E'\rightarrow E,\oh'\rightarrow\oh)
    \psi^{i}(\rvec,E',\oh') \\ & \quad \quad \quad+ Q(\rvec, E, \oh)\;,
  \end{align*}
...
\begin{frame}
  ...
  \item<2-> Combining or using complex acceleration schemes may invalidate
    these assumptions.
...
\begin{frame}
  \frametitle{Project Deliverable}
