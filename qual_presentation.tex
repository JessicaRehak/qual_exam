...
\frame[c]{\frametitle{The multigroup $S_N$ equations}
  To get the standard multigroup $S_N$ equations, we apply the
  ...
  \pause
  \begin{align*}
    &\left[\oh^a \cdot \nabla + \Sigma_{t}^{g}(\rvec)\right]\psi^g(\rvec,
      \oh^a) \\
    & \quad \quad = \sum_{g' = 0}^{G}\sum_{\ell = 0}^N\sum_{m =-\ell}^{\ell}\Sigma_{s,\ell}^{g' \to g}
      Y_{\ell,m}(\oh^a)\phi_{\ell, m}^{g'}(\rvec) + Q^g(\rvec, \oh^a)
  \end{align*}
  In operator form:
  \begin{align*}
    \mat{L}^g\mat{\Psi}^g = \mat{M}\sum_{g' = 0}\mat{S}^{g' \to g}\mat{\Phi}^{g'}
    + \mat{Q}^g, \quad \mat{\Phi}^{g} = \mat{D}\mat{\Psi}^g
  \end{align*}
