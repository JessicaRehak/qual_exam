...
\frame[c]{\frametitle{Problem}

\begin{block}{Problem Statement}
In the presence of up-scattering and a substantial scattering
cross-section, Gauss-Seidel source-iteration can converge arbitrarily
slow because the error in diffuse, persistent modes after each iteration reduces by a factor of
$\Sigma_s/\Sigma_t$.    
\end{block}
...
\note{ This motivates the development of acceleration schemes to speed
  ...
  problems where scattering is dominant}
  
}
    
\frame[c]{\frametitle{Two-grid acceleration}

To mitigate this issue Adams and Morel~\cite{Adams1993} developed the
two-grid method which rests on two assumptions:
\begin{itemize}
\item<1-> The persistent error modes can be accurately determined by a
  course-grid approximation.
\item<2-> Solving this course-grid approximation is more economical
  than solving the actual equation.
\end{itemize}
\onslide<3->{
\begin{block}{Two-grid Acceleration}
  Solve for the error using a course-grid approximation, and use it as
...
\section{Analyzing Acceleration Schemes}

\begin{frame}
  \frametitle{State of the art}
  For an acceleration scheme to be worthwhile, it must remove more
...
\frame{
  ...
  \note{
    ...
    workings of acelleration schemes, but we need to disaggregate the
...
\begin{frame}
  ...
  schemes include:
  \begin{itemize}
  \item<1-> Our definition of work is based on assumptions about algorithm efficiency.
  \item<2-> Combining or using complex acelleration schemes may invalidate
    these assumptions.
  \item<3-> Implementing new schemes can be complicated, making it
    difficult to disaggregate implementation from theory. 
  \item<4-> It can be difficult to reproduce results when accelerated
...
\frame{
  ...
  BART is a new code in development designed with features to overcome
  some of the challenges of analyzing acceleration methods. Four major
  design goals include:
  \begin{enumerate}
  \item<1-> Leverage polymorphism to make implementing new methods
    easier and limit the code needed to do so.
  \item<2-> Include tools to analyze the effectiveness of
    acceleration schemes.
  \item<3-> Provide a framework for users to experiment with
    novel combinations of and modifications to existing acceleration
    schemes.    
  \item<4-> Utilize modern coding and tests practices to make it
...
\begin{frame}
  ...
  \item<4-> Enables us to compare the implementation of the method to
    disaggregate the computer science from the method itself.
  \end{itemize}
...
\onslide<2->{BART will include the ability to \textit{instrument} different parts
...
\begin{frame}
  ...
  \begin{enumerate}[<+->]
  \item A new \texttt{C++} code that solves the continuous
    finite-element discretization of second-order transport equations.
  \item Implementation of a novel acceleration method that
    utilizes NDA and TG, NDA-TG.\@
  \item An in-depth analysis of the three acceleration methods (NDA, TG,
    NDA-TG) using novel instrumentation implemented in the code.
  \end{enumerate}
\end{frame}
...
\begin{frame}
  ...
  \item Addition of Discontinuous-Galerkin Finite Element Formulations support
    to BART.\@
  \item More acelleration methods implemented to test different combinations.
