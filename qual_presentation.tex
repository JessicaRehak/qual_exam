...
\end{block}
\pause
Some acceleration schemes that have been developed:
\begin{itemize}
\item Diffusion synthetic acceleration (DSA).
\item Nonlinear diffusion acceleration (NDA).
...
\begin{frame}
  ...
  \item An in-depth analysis of this acceleration methods using
...
\frame[c]{\frametitle{Conclusion (Reprise)}
...
\item Nonlinear diffusion acceleration (NDA).
\item Diffusion and transport two-grid methods (TG, TTG).
\item A novel combination of NDA and TG.
\end{itemize}
}
}
